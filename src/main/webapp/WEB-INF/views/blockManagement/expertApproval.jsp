                    ...
                    <table class="table-control">
                        ...
                        <tbody>
							<c:forEach var="expertApprovalList" items="${expertApprovalList}">
								<tr>
									<input type="hidden" id="df_num" name="df_num" value="${expertApprovalList.df_num}">
									...
									<td>${expertApprovalList.app_hnd_area}</td>
									...
									<c:if test="${department eq '품질' || department eq '기술'}">
										<td>
											<button type="button" onclick="deleteSelect('${expertApprovalList.app_num}')"
...
<script>
	...
	function deleteSelect(expAppNum) {
		const tableName="expertAppro"
		if (confirm("정말 삭제하십니까?")) {
			window.location.href = "${contextPath}/blockManagement/deleteSelect.do?tableName="+tableName+"&primaryKey="+expAppNum;
		}
	}
</script>
</html>
